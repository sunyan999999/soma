...
def read_version() -> str:
    ...
    if not m:
        print("[FAIL] 无法从 pyproject.toml 解析版本号")
        sys.exit(1)
    return m.group(1)
...
def main():
    ...
    if tag_exists(version):
        print(f"[OK] 版本 {version} → 标签 {tag} 存在")
    else:
        print(f"[FAIL] 版本 {version} → 标签 {tag} 不存在")
        errors.append("tag")

    # ── 检查2: 目录新鲜度 ──
    if errors:
        # 标签不存在则跳过新鲜度检查（没基准点）
        print("  [WARN] 跳过目录新鲜度检查（标签缺失）")
    else:
        stale = check_dir_freshness(tag, version)
        if stale:
            print(f"[WARN] 以下目录的最后提交早于 {tag}，GitHub 文件列表可能显示旧版本号：")
            for d in stale:
                r = run(["git", "log", "-1", "--format=%h %s", "--", d])
                print(f"    {d}/ → {r.stdout.strip()}")
            print("  （不影响推送，仅提示）")
        else:
            print(f"[OK] 关键目录新鲜度 ok")

    # ── 检查3: GitHub Release ──
    if release_exists(version):
        print(f"[OK] GitHub Release {tag} 存在")
    else:
        print(f"[FAIL] GitHub Release {tag} 不存在")
        errors.append("release")

    # ── 汇总 ──
    if not errors:
        print(f"\n[OK] 版本一致性检查全部通过 ({version})")
        return 0
    ...
    if fix_mode:
        ...
        if "tag" in errors:
            ...
            print(f"  [OK] 已创建标签 {tag}")
            r2 = run(["git", "push", "origin", tag])
            if r2.returncode == 0:
                print(f"  [OK] 已推送标签 {tag}")
            else:
                print(f"  [WARN] 标签推送失败: {r2.stderr.strip()}")
        ...
        if "release" in errors:
            ...
            if r2.returncode == 0:
                ...
                print(f"  [WARN] Release 创建失败: {r2.stderr.strip()}")
    ...
    # ── 阻断推送 ──
    print(f"\n{'─'*50}")
    print(f"[WARN] 推送已阻断。缺少: {', '.join(errors)}")
