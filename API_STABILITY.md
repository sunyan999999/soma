...
| `.respond(problem, user_id)` | `str → str` | 完整智者管道，返回合成答案。user_id 可选，传入即启用记忆隔离 |
| `.chat(problem, user_id)` | `str → dict` | 结构化对话，返回 foci/activated/answer/weights |
| `.remember(content, context, importance, user_id, session_id)` | `→ str` | 写入情节记忆，user_id/session_id 可选 |
| `.remember_semantic(subject, predicate, object_, confidence, namespace)` | `→ None` | 写入语义三元组，namespace 可选实现隔离 |
